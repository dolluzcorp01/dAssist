...
function Employee_Center() {
    ...
    const handleToggleActive = (emp) => {
        ...
        Swal.fire({
            title: "Are you sure?",
            text: emp.is_active === 1
                ? "This will make the employee inactive."
                : "This will make the employee active.",
            icon: "warning",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                try {
                    await apiFetch(`/api/employee/toggle-active/${emp.emp_id}`, {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                            updated_by: loggedInEmpId,
                    ...
                    Swal.fire(
                        "Updated!",
                        `Employee has been ${emp.is_active === 1 ? "deactivated" : "activated"}.`,
                        "success"
                    );
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr
                                            key={emp.emp_id}
                                            style={{
                                                backgroundColor: emp.is_active === 0 ? "#f0f0f0" : "transparent",
                                                color: emp.is_active === 0 ? "#888" : "inherit"
                                            }}
                                        >
                                            <td>{emp.emp_id}</td>
                                            ...
                                            <td style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                                ...
                                                <label
                                                    className="switch"
                                                    title={emp.is_active === 1 ? "Make Employee Inactive" : "Make Employee Active"}
                                                >
                                                    <input
                                                        type="checkbox"
